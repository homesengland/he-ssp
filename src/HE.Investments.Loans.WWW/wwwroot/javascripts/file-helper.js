...
(async () => {
  ...
  const fileUploadFinished = async (fileId, fileName, response) => {
    return response.json()
      .then(uploadedFile => {
        ...
        if (uploadedFile.canBeRemoved && removeFileUrlElement) {
          const removeUrl = removeFileUrlElement.value.replace('%3AfileName', fileName);
        ...
        if (downloadFileUrlElement) {
          const downloadUrl = downloadFileUrlElement.value.replace('%3AfileName', fileName);
