...
public class Order16ChargesDebtIntegrationTests : IntegrationTest
{
    ...
    {
        // given
        var chargesDebtPage = await TestClient.NavigateTo(ProjectPagesUrls.ChargesDebt(UserOrganisationData.OrganisationId, _applicationLoanId, _projectId));
        var continueButton = chargesDebtPage.GetGdsSubmitButtonById("continue-button");

        // when
        chargesDebtPage = await TestClient.SubmitButton(
            continueButton,
            (nameof(ProjectViewModel.ChargesDebt), CommonResponse.Yes),
            (nameof(ProjectViewModel.ChargesDebtInfo), string.Empty));
    ...
    public async Task Order02_ShouldShowValidationMessage_WhenYesIsSelectedAndAdditionalInformationIsTooLong()
    {
        ...
        chargesDebtPage = await TestClient.SubmitButton(
            continueButton,
            (nameof(ProjectViewModel.ChargesDebt), CommonResponse.Yes),
            (nameof(ProjectViewModel.ChargesDebtInfo), TextTestData.TextThatExceedsLongInputLimit));
    ...
    public async Task Order03_ShouldRedirectToAffordableHomes_WhenYesIsSelectedAndCorrectAdditionalInformationIsProvided()
    {
        ...
        // when
        var affordableHomesSuffix = await TestClient.SubmitButton(
            continueButton,
            (nameof(ProjectViewModel.ChargesDebt), CommonResponse.Yes),
            (nameof(ProjectViewModel.ChargesDebtInfo), TextTestData.TextThatNotExceedsLongInputLimit));
