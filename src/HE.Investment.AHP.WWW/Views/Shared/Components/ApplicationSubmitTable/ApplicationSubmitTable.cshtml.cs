...
public class ApplicationSubmitTable : ViewComponent
{
    public Task<IViewComponentResult> InvokeAsync(ApplicationSubmitModel model)
    {
        ...
        {
            new(
                model.ApplicationId, [new(model.SiteName), new($"{model.ApplicationName}: {model.Tenure}"), new(model.NumberOfHomes), new(model.FundingRequested), new(model.TotalSchemeCost),]),
        };
