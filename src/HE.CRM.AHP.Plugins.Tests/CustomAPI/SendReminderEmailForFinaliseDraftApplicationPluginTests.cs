    ...
    public class SendReminderEmailForFinaliseDraftApplicationPluginTests
    {
        ...
        public void Test_1()
        {
            ...
            fakedContext.Initialize(new List<Entity>()
            {
                ...
                { new invln_notificationsetting { Id = Guid.NewGuid(), invln_subject = "subject", invln_templateid = "tempalte id", invln_templatetypename = "AHP_EXTERNAL_REMINDER_TO_FINALIZE_DRAFT_APPLICATION" } }
            });

            pluginContext.InputParameters = new ParameterCollection
            {
                { invln_sendremindertofinalisadraftapplicationRequest.Fields.invln_applicationid, appId.ToString()}
            };
            try
            {
