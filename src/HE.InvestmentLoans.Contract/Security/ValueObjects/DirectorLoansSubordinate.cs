    ...
    public DirectorLoansSubordinate(bool canBeSubordinated, string reasonWhyCannotBeSubordinated)
    {
        ...
        if (!CanBeSubordinated)
        {
            ...
            if (reasonWhyCannotBeSubordinated.Length > MaximumInputLength.LongInput)
            {
                OperationResult
                    .New()
                    .AddValidationError(nameof(CanBeSubordinated), ValidationErrorMessage.LongInputLengthExceededFor(FieldNameForInputLengthValidation.ReasonWhyCannotBeSubordinated))
                    .CheckErrors();
            }
