    ...
    protected PrivateSectorFunding(bool isApplied, string? privateSectorFundingApplyResult, string? privateSectorFundingNotApplyingReason)
    {
        if (isApplied)
        {
            ...
            else if (privateSectorFundingApplyResult!.Length >= MaximumInputLength.LongInput)
            {
                OperationResult
                .New()
                .AddValidationError(nameof(FundingViewModel.PrivateSectorFundingResult), ValidationErrorMessage.InputLongerThanThousandCharacters)
                .CheckErrors();
            }
        }
        else
        {
            ...
            else if (privateSectorFundingNotApplyingReason!.Length >= MaximumInputLength.LongInput)
            {
                OperationResult
                .New()
                .AddValidationError(nameof(FundingViewModel.PrivateSectorFundingReason), ValidationErrorMessage.InputLongerThanThousandCharacters)
                .CheckErrors();
            }
        }
