...
namespace HE.Base.Plugins
{
    ...
    /// </summary>
    /// <typeparam name="TContext">The type of the context inherited from OrganizationServiceContext.</typeparam>
    /// <seealso cref="Microsoft.Xrm.Sdk.IPlugin" />
    public abstract class PluginBase<TContext> : IPlugin where TContext : OrganizationServiceContext
    {
        /// <summary>
        /// Plugin settings
        /// </summary>
        ...
        protected PluginSettings LoadSettings(string settings)
        {
            ...
            try
            {
                ...
                var streamArray = Encoding.UTF8.GetBytes(settings);

                using (var ms = new MemoryStream(streamArray))
                {
                    var pluginSettings = (PluginSettings) serializer.ReadObject(ms);
