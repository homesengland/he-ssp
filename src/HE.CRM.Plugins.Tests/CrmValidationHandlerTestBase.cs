...
namespace PwC.Base.Tests.Plugins.Handlers
{
    ...
    public abstract class CrmValidationHandlerTestBase<TEntity, THandler, TContext> : CrmHandlerTestBase<THandler, TContext>
        where TEntity : Entity, new()
        where THandler : ICrmValidationHandler
        where TContext : OrganizationServiceContext
    {
        ...
        public override void Asset(string messageName, int messageStage, HandlerCache cache = null)
        {
            ...
            if (PostImage != null)
            {
                fakedPluginContext.InputParameters.Add(nameof(PostImage), PostImage);
            }
        ...
        /// <param name="cache">Handler cache. It's passed across handlers within specified plugin</param>
        public override void Asset(string messageName, CrmProcessingStepStages messageStage, HandlerCache cache = null)
        {
            this.Asset(messageName, (int)messageStage, cache);
        }
    }
}
