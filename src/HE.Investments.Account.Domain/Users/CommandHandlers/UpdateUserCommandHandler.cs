    ...
    where TCommand : IUpdateUserCommand, IRequest<OperationResult>
{
    ...
    public async Task<OperationResult> Handle(TCommand request, CancellationToken cancellationToken)
    {
        ...
        var entity = await _repository.GetUser(request.UserId, userAccount.OrganisationId(), cancellationToken);

        Update(entity, request);

        await _repository.Save(entity, userAccount.OrganisationId(), cancellationToken);
