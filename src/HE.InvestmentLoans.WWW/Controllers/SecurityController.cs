...
public class SecurityController : Controller
{
    ...
    {
        var model = await _mediator.Send(new BL._LoanApplication.Queries.GetSingle() { Id = id });
    ...
    {
        model = await this._mediator.Send(new BL._LoanApplication.Queries.GetSingle() { Id = id });
        var workflow = new SecurityWorkflow(model, _mediator);
        workflow.NextState(Trigger.Back);
